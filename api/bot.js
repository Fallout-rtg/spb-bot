...
/set_reply <ссылка на сообщение> — выбрать сообщение для ответа (только админ)
/clear_reply — сбросить выбранное сообщение
...
// —————————— Система ответа по ссылке ——————————
let replyTargets = {};

bot.command('set_reply', restrictedCommand(async (ctx) => {
  const args = ctx.message.text.split(' ');
  if (args.length < 2) return ctx.reply('❌ Укажите ссылку на сообщение.');

  const url = args[1];
  const match = url.match(/t\.me\/c\/(\d+)\/(\d+)/);
  if (!match) return ctx.reply('❌ Неверный формат ссылки. Используйте ссылку вида t.me/c/<chatId>/<msgId>.');

  const chatId = `-100${match[1]}`;
  const msgId = parseInt(match[2], 10);

  replyTargets[ctx.from.id] = { chatId, msgId };
  await ctx.reply(`✅ Ссылка сохранена. Теперь отправьте сообщение (текст, фото, видео и т.д.), чтобы бот переслал его в ответ.`);
}, { adminOnly: true }));

bot.command('clear_reply', restrictedCommand(async (ctx) => {
  delete replyTargets[ctx.from.id];
  await ctx.reply('✅ Сохранённая ссылка сброшена.');
}, { adminOnly: true }));
...
bot.on('message', safeHandler(async (ctx) => {
  ...
  // --- Ответ админа пользователю (без упоминания автора) ---
  if (isAdminUser && chatId === ADMIN_CHAT_ID && message.reply_to_message) {
    let originalId = null;
    ...
    if (!originalId || Number.isNaN(originalId)) {
      await ctx.reply('❌ Не удалось определить пользователя. Ответьте на пересланное сообщение или на сообщение с ID.');
      return;
    }
  ...
  // --- Система ответа по ссылке ---
  if (isAdminUser && replyTargets[userId]) {
    const { chatId: targetChatId, msgId: targetMsgId } = replyTargets[userId];

    try {
      if (message.text) {
        await ctx.telegram.sendMessage(targetChatId, message.text, {
      ...
      if (message.photo) {
        await ctx.telegram.sendPhoto(
          ...
          { caption: message.caption || '', reply_to_message_id: targetMsgId }
        );
      }
      if (message.video) {
      ...
      await ctx.reply('✅ Сообщение отправлено в ответ на выбранное.');
      delete replyTargets[userId];
    } catch (err) {
      console.error('Ошибка при пересылке:', err);
      await ctx.reply(`❌ Ошибка при пересылке: ${err.description || err.message}`);
    }
    return;
  }

  // --- Пересылка сообщений пользователей в админский чат ---
  if (!isAdminUser && isPrivate(ctx) && !message.text?.startsWith('/')) {
