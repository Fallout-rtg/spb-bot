...
// ----------------- Обработчик: пересылка сообщений ЛС в админский чат -----------------
bot.on('message', safeHandler(async (ctx) => {
  ...
  if (!isAdmin(ctx) && isPrivate(ctx) && !message.text?.startsWith('/')) {
    ...
    const sentMessage = await ctx.forwardMessage(ADMIN_CHAT_ID, chatId, message.message_id);
    MESSAGE_MAP.set(sentMessage.message_id, userId);

    await ctx.telegram.sendMessage(ADMIN_CHAT_ID, caption, { parse_mode: 'HTML', disable_web_page_preview: true });
  }
}));

// ----------------- Обработчик: ответ админа пользователю без упоминания -----------------
bot.on('message', safeHandler(async (ctx) => {
  const message = ctx.message;
  const chatId = message.chat.id;
  ...
  if (isAdminUser && chatId === ADMIN_CHAT_ID && message.reply_to_message) {
    const originalId = MESSAGE_MAP.get(message.reply_to_message.message_id);
    if (!originalId) {
      await ctx.reply(
        '❌ Не удалось определить пользователя. Используйте пересланное сообщение или ссылку на сообщение t.me/c/...'
      );
      return;
    }
...
// ----------------- Обработка сообщений канала для комментариев -----------------
bot.on('message', safeHandler(async (ctx) => {
