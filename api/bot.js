...
// —————————— Команды управления разрешёнными чатами ——————————
bot.command('ida', safeHandler(async (ctx) => {
  if (!ADMIN_IDS.includes(ctx.from.id)) return ctx.reply('❌ Только админам.');
  const args = ctx.message.text.split(' ');
...
// —————————— Текст комментариев под постами ——————————
const COMMENT_TEXT = `
...
📡 Наш чат: <a href="https://t.me/+qAcLEuOQVbZhYWFi">ТГ</a> | <a href="https://discord.gg/rBnww7ytM3">Discord</a> | <a href="https://www.tiktok.com/@spectr_mindustry?_t=ZN-8yZCVx33mr9&_r=1">TikTok</a>
`;
...
// —————————— Команда для просмотра текста комментариев ——————————
bot.command('comment_text', safeHandler(async (ctx) => {
  if (!ADMIN_IDS.includes(ctx.from.id)) return ctx.reply('❌ Только админам.');
  await ctx.reply(COMMENT_TEXT, {
    parse_mode: 'HTML',
